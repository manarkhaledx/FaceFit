...
class ObjectRenderer {
    ...
    fun createOnGlThread(context: Context, objInputStream: InputStream, diffuseTextureAssetName: String) {
        // Compiles and loads the shader based on the current configuration.
        compileAndLoadShaderProgram(context)

        // Read the texture.
        val textureBitmap = if (diffuseTextureAssetName.startsWith("/")) {
            // It's a file path from backend
            BitmapFactory.decodeFile(diffuseTextureAssetName)
        } else {
            // It's an asset path
